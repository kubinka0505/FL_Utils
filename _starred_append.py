...
if CLI:
	...
	prompt_title = "Select directory containing files to star"
...
def add_to_starred(files_dir: str, allowed_extensions: tuple = None, recursive: bool = True, quiet: bool = True, dir_fl_data: str = None):
	"""
	Appends files to the FL Studio "STARRED" tab.

	Args:
		files_dir: Directory containing files to be added..
	...
	for File in Path(files_dir).glob(Wildcard):
		...
		for File in Files:
			...
			if Line.lower() in Starred_Content.lower():
				if not quiet:
					...
					Message = f'[{Counter_STR}/{Amount}] "{File_Display}" is already starred'
				...
				if not quiet:
					...
					Message = f'[{Counter_STR}/{Amount}] "{File_Display}" has been starred'
					Message += " " * (Width - len(Message) - 1)
	...
	if not quiet:
		print()
		print(f"Starred {Counter} files in {str(RunTime)[2:-3]}s")
